...
//
// Constructs one instance of six of the seven UoPs in GROCERY_with_IM.face,
// drives each through the full FACE LCM sequence (Configure -> Initialize ->
// Framework_Connect -> Framework_Disconnect -> Finalize), and exits.
//
// Charlie_Comp is not included here -- see "Why Charlie_Comp isn't in the
// integration binary" in README.md: it shares NewStockAgent_Request with
// Beta_Comp under two incompatible Traits<> bindings (requester vs.
// responder role of the same CLIENT_SERVER connection), which cannot both
// be linked into one process with today's type-trait design. Charlie_Comp
// still builds as its own Charlie_Comp_impl library.
//
